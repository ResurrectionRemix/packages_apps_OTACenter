...
import com.mahdi.mbq.app.main.AwesomeClass;
...
public class UpdateChecker extends AsyncTask<Context, Integer, String> {
	...
	public static boolean connectivityAvailable(Context context) {
		...
		return (netInfo != null && netInfo.isConnectedOrConnecting() && (netInfo
				.getType() == ConnectivityManager.TYPE_MOBILE || netInfo
	...
	protected String doInBackground(Context... arg) {
		mContext = arg[0];
		Message msg;
		if (mContext != null && mContext.toString().contains("MahdiCenter")) {
			msg = mHandler.obtainMessage(MSG_CREATE_DIALOG);
			mHandler.sendMessage(msg);
		}
		HttpURLConnection urlConnection = null;
		if (!connectivityAvailable(mContext))
			return "connectivityNotAvailable";
		try {
			getDeviceTypeAndVersion();
			if (mNoLog == false)
				Log.d(TAG, "strDevice=" + strDevice + "   CurVer=" + CurVer);
			if (strDevice == null || CurVer == null)
				return null;
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_DOCUMENT) {
					...
					if (xpp.getName().equalsIgnoreCase(strDevice))
						tagMatchesDevice = true;
					else if (tagMatchesDevice
							&& xpp.getName().equalsIgnoreCase("Filename"))
						inFileName = true;
					else if (tagMatchesDevice
							&& xpp.getName().equalsIgnoreCase("DownloadUrl"))
						inDownloadURL = true;
					...
					if (tagMatchesDevice && inFileName) {
						...
						try {
							...
							putDataInprefs(mContext, "Filename",
									versionOnServer);
							if (versionOnServer.compareToIgnoreCase(CurVer) > 0)
								newFileName = tempFileName;
						} catch (Exception invalidFileName) {
							Log.e(TAG, "File Name from server is invalid : "
									+ tempFileName);
						}
	...
	private void showInvalidLink() {
		if (mContext != null && mContext.toString().contains("MahdiCenter")) {
			Message msg = mHandler.obtainMessage(MSG_DISPLAY_MESSAGE,
					mContext.getString(R.string.bad_url));
			mHandler.sendMessage(msg);
		} else {
	...
	private void showNotification() {
		...
		Intent intent = new Intent(mContext, AwesomeClass.class);
